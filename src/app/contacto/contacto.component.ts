...
export class ContactoComponent {
  ...
  enviarCorreco(event: any) {
    if (event.valid) {
      ...
      Swal.showLoading();
      let body = {
        Nombre: this.correo.name,
        Correo: this.correo.email,
        Comentarios: this.correo.comments,
      }

      this.peticiones.postNoToken2(body, 'https://formsubmit.co/ajax/oliverhernandez206@gmail.com').then((resp: any) => {
        if (resp.success) {
          Swal.close();
          this.correo.name = '';
          this.correo.email = '';
